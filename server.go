...
func (s *Server) handleRevokeSession(w http.ResponseWriter, r *http.Request) {
	...
	// Generate a sessions page token so the user can navigate back
	sessionsTs := fmt.Sprintf("%d", time.Now().Unix())
	sessionsTok := computeCSRFToken(s.cfg.SharedSecret, username, sessionsTs)
	sessionsLink := fmt.Sprintf("%s/sessions?u=%s&ts=%s&tok=%s",
		strings.TrimRight(s.cfg.ExternalURL, "/"),
		template.URLQueryEscaper(username), sessionsTs, sessionsTok)
...
<head>
  ...
  <style>`+sharedCSS+`
    ...
    .back-link { display: inline-block; margin-top: 16px; color: var(--primary); text-decoration: none; font-weight: 600; font-size: 0.938rem; }
...
<body>
  <div class="card">
    ...
    <p><a href="`+template.HTMLEscapeString(sessionsLink)+`" class="back-link">Back to sessions</a></p>
  </div>
</body>
</html>`)
}
...
// GET /sessions
// sessionsTokenTTL is how long a sessions page token is valid (refreshable window).
...
func (s *Server) handleSessionsPage(w http.ResponseWriter, r *http.Request) {
	...
	// Check for a valid session token (set after OIDC callback, allows refresh)
	username := r.URL.Query().Get("u")
	ts := r.URL.Query().Get("ts")
	tok := r.URL.Query().Get("tok")
	if username != "" && ts != "" && tok != "" && validUsername.MatchString(username) {
		tsInt, err := strconv.ParseInt(ts, 10, 64)
		if err == nil && time.Since(time.Unix(tsInt, 0)).Abs() < sessionsTokenTTL {
			expected := computeCSRFToken(s.cfg.SharedSecret, username, ts)
			if subtle.ConstantTimeCompare([]byte(expected), []byte(tok)) == 1 {
				s.renderSessionsList(w, username)
				return
			}
		}
	}
...
func (s *Server) handleSessionsCallback(w http.ResponseWriter, r *http.Request) {
	...
	// Redirect to /sessions with a refreshable token instead of rendering directly.
	// This way the user can refresh the page without replaying the OIDC code.
	ts := fmt.Sprintf("%d", time.Now().Unix())
	tok := computeCSRFToken(s.cfg.SharedSecret, username, ts)
	sessionsURL := fmt.Sprintf("%s/sessions?u=%s&ts=%s&tok=%s",
		strings.TrimRight(s.cfg.ExternalURL, "/"), username, ts, tok)
	http.Redirect(w, r, sessionsURL, http.StatusSeeOther)
}

// renderSessionsList renders the sessions list page for an authenticated user.
func (s *Server) renderSessionsList(w http.ResponseWriter, username string) {
...
<head>
  ...
  <style>` + sharedCSS + `
    ...
    .revoke-btn { background: none; border: 1px solid var(--danger); color: var(--danger); padding: 10px 16px; border-radius: 8px; cursor: pointer; font-size: 0.813rem; font-weight: 600; min-height: 44px; white-space: nowrap; flex-shrink: 0; }
